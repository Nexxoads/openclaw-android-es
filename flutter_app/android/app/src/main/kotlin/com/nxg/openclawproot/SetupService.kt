...
class SetupService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        startForeground(NOTIFICATION_ID, buildNotification("Configurando entorno...", -1))
        if (isRunning) {
            return START_STICKY
        }
        isRunning = true
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                "OpenClaw ES — instalación",
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = "Progreso de la instalación del entorno OpenClaw ES"
            }
    ...
    private fun buildNotification(text: String, progress: Int): Notification {
        ...
        builder.setContentTitle("OpenClaw ES — instalación")
            .setContentText(text)
