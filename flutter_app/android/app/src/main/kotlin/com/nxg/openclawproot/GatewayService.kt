...
class GatewayService : Service() {
        ...
        gatewayThread = Thread {
            try {
                ...
                if (isRunning && restartCount < maxRestarts) {
                    ...
                    updateNotification("Reiniciando en ${delayMs / 1000}s (intento $restartCount)...")
                    ...
                    updateNotification("Gateway detenido (falló)")
                    isRunning = false
                }
            } catch (e: Exception) {
                if (!stopping) {
                    ...
                    isRunning = false
                    updateNotification("Error del gateway")
                }
            }
        }.also { it.start() }
    }
    ...
    private fun updateNotificationRunning() {
        updateNotification("Puerto 18789 \u2022 ${formatUptime()}")
    }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Mantiene el gateway en segundo plano"
            }
    ...
    private fun buildNotification(text: String): Notification {
        ...
        builder.setContentTitle("OpenClaw ES — Gateway")
            .setContentText(text)
