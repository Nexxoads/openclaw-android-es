  ...
  Future<void> init() async {
    ...
    if (alreadyRunning) {
      ...
      _updateState(_state.copyWith(
        ...
        logs: [..._state.logs, _ts('[INFO] Proceso del gateway detectado, reconectando...')],
      ));
  ...
  void _subscribeLogs() {
    ...
    _logSubscription = NativeBridge.gatewayLogStream.listen((log) {
      ...
      if (urlMatch != null) {
        ...
        NativeBridge.showUrlNotification(dashboardUrl!, title: 'Panel listo');
      }
...
''';
  ...
  Future<void> start() async {
    ...
    _updateState(_state.copyWith(
      ...
      logs: [..._state.logs, _ts('[INFO] Iniciando gateway...')],
    ));
    ...
    try {
      ...
      _updateState(_state.copyWith(
        ...
        logs: [..._state.logs, _ts('[ERROR] Error al iniciar: $e')],
      ));
    } finally {
  ...
  Future<void> stop() async {
    ...
    try {
      ...
      _updateState(GatewayState(
        ...
        logs: [..._state.logs, _ts('[INFO] Gateway detenido')],
      ));
    } catch (e) {
      _updateState(_state.copyWith(
        status: GatewayStatus.error,
        errorMessage: 'Error al detener: $e',
      ));
    }
  }
    ...
    try {
      ...
      if (response.statusCode < 500 && _state.status != GatewayStatus.running) {
        ...
        _updateState(_state.copyWith(
          ...
          logs: [..._state.logs, _ts('[INFO] Gateway en buen estado')],
        ));
      }
    } catch (_) {
      ...
      if (!isRunning && _state.status != GatewayStatus.stopped) {
        ...
        if (_startingAt != null &&
          ...
          _updateState(_state.copyWith(
            logs: [..._state.logs, _ts('[INFO] Iniciando, esperando al gateway...')],
          ));
          return;
        }
        _updateState(_state.copyWith(
          status: GatewayStatus.stopped,
          logs: [..._state.logs, _ts('[AVISO] El proceso del gateway no está en ejecución')],
        ));
        _cancelAllTimers();
      }
    }
